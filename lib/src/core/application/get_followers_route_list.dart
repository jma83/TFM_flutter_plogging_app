...
class GetFollowersRouteList {
  ...
  execute(String currenUserId) async {
    final List<String> userFollowingIds = await _getFollowingIds(currenUserId);
    final List<RouteData> userFollowingRoutes =
        await _getFollowingRoutes(userFollowingIds);
    final List<LikeData> likes =
        await _matchRoutesWithUserLikes(userFollowingRoutes, currenUserId);
    return RouteListData.createListFromRoutesAndLikes(
        userFollowingRoutes, likes);
  }

  Future<List<String>> _getFollowingIds(String currenUserId) async {
